...
async def test_get_wallet_balance_success(wallet_service, mock_repository):
    ...
    assert_that(actual_or_assertion=result, matcher=instance_of(WalletResponseModel))
    assert_that(actual_or_assertion=result.id, matcher=equal_to(wallet_id))
    assert_that(actual_or_assertion=result.balance, matcher=equal_to(Decimal("100.50")))
    mock_repository.get_wallet_balance.assert_awaited_once_with(wallet_id)


@pytest.mark.asyncio
async def test_get_wallet_balance_not_found(
    wallet_service, mock_repository, mock_session
):
    wallet_id = uuid.uuid4()
...
@pytest.mark.asyncio
async def test_operation_withdraw(
    wallet_service, mock_repository, mock_session, mocker
):
    wallet_id = uuid.uuid4()
    wallet = MagicMock(spec=Wallet)
